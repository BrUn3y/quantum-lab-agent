...
Model: configurable via LAB_MODEL (Granite/Ollama or Watsonx)
Port: 8000
...
⚠️⚠️⚠️ CRITICAL RULE FOR quantum_computing_client - NEVER VIOLATE ⚠️⚠️⚠️

When executing code with quantum_computing_client, your response MUST ALWAYS include:
1. ✅ **Job ID** (ABSOLUTELY MANDATORY) - User NEEDS this to query the job
2. ✅ Backend used
3. ✅ Results (if available)
4. ✅ Measurements table (if available)

⚠️ THE JOB ID IS THE MOST IMPORTANT PIECE OF INFORMATION ⚠️
Without the Job ID, the user CANNOT check their job status or results.

MANDATORY RESPONSE FORMAT after execution:
```
✅ Circuit executed successfully!

**Job ID: [ACTUAL_JOB_ID_HERE]**
Backend: [backend_name]
Shots: [number]

💡 To check the status and results of your job, use:
"What is the status of job [ACTUAL_JOB_ID_HERE]?"
```
...
💡 To check the status and results, use:
'What is the status of job d671cklbujdc73cvbp30?'"

INCORRECT EXAMPLES (ABSOLUTELY FORBIDDEN):
❌ "The circuit was successfully executed on ibm_torino." (MISSING JOB ID!)
❌ "Execution completed. Check the results later." (MISSING JOB ID!)
❌ "Your circuit is running on ibm_torino." (MISSING JOB ID!)

⚠️ IF YOU DON'T INCLUDE THE JOB ID, THE USER CANNOT ACCESS THEIR RESULTS ⚠️
...
_EXECUTE_PATTERN = re.compile(
    r"\b(execut\w*|run|running|test\w*|submit\w*|ejec(?:u|ú)t\w*|corre\w*|prueb\w*|prob\w*)\b",
    re.IGNORECASE,
)
...
def _extract_qasm(response: str) -> str | None:
    ...
    qasm_match = fenced_match or _QASM_PATTERN.search(response)
    if not qasm_match:
        return None
    qasm = qasm_match.group(1) if fenced_match else qasm_match.group(0)
    qasm = re.split(r"\n\s*---\s*\n", qasm, maxsplit=1)[0]
    if "\\n" in qasm:
        qasm = qasm.replace("\\r\\n", "\n").replace("\\n", "\n").replace('\\"', '"')
    return qasm.strip()
...
async def _create_and_execute(request: str) -> tuple[str, str]:
    ...
    execution_request = (
        f"{request}\n\nExecute exactly once. If real hardware was requested without a backend, "
        "select the least busy operational real backend. Otherwise use a simulator by default. "
...
async def quantum_lab_agent(
    ...
    """
    # STEP 0: Store user message in history (best effort)
    try:
        await context.store(input)
    except Exception as error:
        print(f"⚠️ [History] Could not store user message: {error}")
    ...
    if _is_create_and_execute_request(user_query):
        ...
        try:
            ...
            response_message = AgentMessage(
                text=f"{developer_response.rstrip()}\n\n## ⚡ Execution\n\n{computing_response.lstrip()}"
            )
            yield trajectory.trajectory_metadata(
                title="✅ Circuit created and submitted",
                content="QASM validated and sent exactly once to the Computing Agent.",
            )
            yield response_message
            try:
                await context.store(response_message)
            except Exception as error:
                print(f"⚠️ [History] Could not store response: {error}")
        except Exception as error:
            yield AgentMessage(text=f"❌ Could not complete create-and-execute: {explain_error(error)}")
        return
